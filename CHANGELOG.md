...
### Added
- Understanding debt is now estimated automatically from commit signals — no developer
  survey required. Needing a survey was the main reason the metric went uncollected.

## [0.9.5] - 2026-07-11 — "Continuous AI probability"

### Changed
- AI detection returns a continuous, content-based probability instead of a bucketed score.

### Removed
- The dashboard extra, which did not install cleanly.

## [0.9.4] - 2026-07-11

### Fixed
- `codedna ask` degrades gracefully when the Needle model is not installed, instead of failing.

## [0.9.3] - 2026-07-11

### Added
- NatureCo account (SSO) support and `codedna scan --json` for ecosystem integration.

### Fixed
- `scan` returned 0 files on Windows: `git ls-files` emits POSIX paths, which never matched
  the native separators they were compared against. Normalised with `as_posix()`.
